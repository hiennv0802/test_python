...
original_author="hiennv0802"
original_name="test_python"
original_urlname="test_python"
original_description="Awesome test_python created by hiennv0802" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/test_python "${name}"
...
then
    ...
    echo "then run 'test_python' or 'python -m test_python'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'test_python' or 'python -m test_python' or 'poetry run test_python'"
fi
