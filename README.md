...
- 🎯 Entry points to execute your program using `python -m <test_python>` or `$ test_python` with basic CLI argument parsing.
...
---
# test_python Flask Application

Awesome test_python created by hiennv0802
...
```bash
git clone https://github.com/hiennv0802/test_python test_python
cd test_python
make install
```
...
```bash
test_python create-db   # run once
test_python populate-db  # run once (optional)
test_python add-user -u admin -p 1234  # ads a user
test_python run
```
